...
falls, and sleeps. Pure C and GTK3; `ffplay` is optional and only needed for
...
--review-parent N   Show parent animation N with its authored child scene.
...
- `ESHEEP_DEBUG_MONITORS=1`: print live monitor geometry and workarea pairs.
...
The sheep continues to work without a tray area. Character, package,
spritesheet, and pet-count changes take effect immediately and are saved to
the config file.
...
- Full original behavior graph ported from eSheep: 54 original animations,
  plus 13 review-only extension records, with weighted, context-filtered
  transitions and child animations (`src/animations_data.c`, generated by
  `tools/gen_animations.py` from `tools/esheep_animations.xml`). See
  [`docs/animations.md`](docs/animations.md) for the grouped catalog and how
  to review individual animations or multi-sprite scenes.
...
**Package boundary**: built-in sheep and penguin sheets use the shared 16×11
tile grid (176 tiles, 640×440 for sheep, 1280×880 for penguin). A custom
behavior package loaded with `--package` is validated against the runtime
expression grammar and supplies its own tile-grid dimensions; provide a
matching spritesheet with `--sprite` or `spritesheet=`. Use
...
The 26 upstream catalog entries include their behavior XML, icon, and README
metadata in `assets/Pets`. Character spritesheet payloads are not bundled, so
those entries remain unavailable for direct selection until a matching sheet
is supplied. Some packages also contain authored sound records. See
[`packaging/PROVENANCE.md`](packaging/PROVENANCE.md) for provenance and
licensing records.
